...
export class CalculatorService {
  constructor(
    ...
    @InjectModel(Calculation.name) private calculationModel: Model<CalculationDocument>,
  ) {}
  ...
  async updateState(userId: string, memory: number, currentExpression: string) {
    const user = await this.userModel.findByIdAndUpdate(
      userId,
      { 'calculatorState.memory': memory, 'calculatorState.currentExpression': currentExpression },
      { new: true }
    ).exec();
